...
const lineSchema = z.strictObject({
  ...
  stock_check: z.string().max(300).nullish().describe('Why this line is worth checking in the store: Kroger’s rough stock level, or many packages'),
...
export const choiceSchema = z.strictObject({
  plan_id: planId,
  reason: z.string().trim().min(20).max(600).describe('Why this plan is the class’s best, for the class to read: two or three sentences that name the group that created it and say why it beats the others'),
