...
export async function listRecipes(input) {
  ...
  const rows = await getStore().listRecipes(parsed.data);
  const prices = await getStore().pricingsFor([...new Set(rows.map((r) => r.meal_id))]);
  return { ok: true, count: rows.length, recipes: rows.map((r) => ({ ...toRecipe(r), pricing: pricingSummary(prices.get(r.meal_id)) })) };
}
