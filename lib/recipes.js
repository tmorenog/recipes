...
import { getSettings, limitFor, totalLimit } from './settings.js';
import { verifyRecipe } from './mealdb.js';
...
export const listSchema = z.strictObject({
  ...
  priced: z
    ...
    .optional()
    .describe('true: only recipes the Recipe Pricer has priced, with their cost per serving'),
});
...
export function pricingSummary(row) {
  ...
  // "priced" with no cost (an old row priced for no one) can't be used in a plan.
  if (status !== 'priced' || row.cost_per_serving_usd == null) return { status: status === 'priced' ? 'failed' : status };
  return {
    status,
...
export async function saveRecipe({ group, channel, input }) {
  ...
  if (!parsed.success) {
    const errors = reasons(parsed.error, value);
    await record({ group, channel, action: 'save_recipe', ok: false, detail: errors.join('; '), input });
    return { ok: false, status: 400, errors };
  }
  const r = parsed.data;
  // Only real recipes are stored: the coordinator checks this one against TheMealDB.
  const verified = await verifyRecipe(r);
  notes.push(...verified.notes);
  if (verified.errors.length) {
    await record({ group, channel, action: 'save_recipe', ok: false, detail: verified.errors.join('; '), input });
    return { ok: false, status: 400, errors: verified.errors };
  }
  ...
  }, { maxPicks: (await getSettings()).max_recipes_per_group });
  if (res.limit != null) {
    const limited = totalLimit('recipe', res.limit, (await getSettings()).max_recipes_per_group);
    await record({ group, channel, action: 'save_recipe', ok: false, detail: limited.errors[0], input });
    return { ok: false, ...limited };
  }
  if (res.duplicate) {
...
export async function listRecipes(input) {
  ...
  const recipes = (await getStore().listRecipes(parsed.data)).map(toRecipe);
