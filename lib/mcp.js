...
Meal Planner: read priced recipes with list_recipes (priced: true); check a draft of five
dinners, Monday to Friday, with check_meal_plan; then save it with save_meal_plan.
...
const TOOLS = {
  ...
  planner: ['get_expectations', 'list_recipes', 'check_meal_plan', 'save_meal_plan'],
};
...
export function createMcpServer(group, { agent = null } = {}) {
  ...
  const planDescription =
    'Five dinners, Monday to Friday, from priced recipes, with the budget (US dollars per person for the week). ' +
    'The coordinator looks up each dinner’s cost and nutrition from the Pricer, adds up the week, and checks the balance rules.';
