...
import { cartSchema, choiceSchema, listPlansForShopper, buildWeekCart, saveChoice } from './shopper.js';
...
Shopper: compare the saved plans with list_meal_plans, preview a plan's Kroger cart with
build_week_cart, and store the class's choice with save_choice; see /shopper.
...
const TOOLS = {
  ...
  shopper: ['get_contract', 'list_meal_plans', 'build_week_cart', 'search_kroger_products', 'save_choice'],
};
...
export function createMcpServer(group, { agent = null } = {}) {
  ...
  server.registerTool(
    'get_contract',
    {
      ...
      inputSchema: z.strictObject({
        agent: z.enum(AGENTS).optional().describe(agent ? `Defaults to "${agent}"` : '"scout" (Recipe Scout), "planner" (Meal Planner) or "shopper" (Shopper)'),
      }),
  ...
  server.registerTool(
    'list_recipes',
    {
      ...
      description:
        (agent === 'planner'
          ? 'Recipes from every group, newest first: all of them (up to 500) unless you pass status or limit. '
          : 'Recipes from every group, newest first. By default only "new" ones, which nobody has marked processed. ') +
        'Each recipe is stored once, with pick_count and picked_by (the groups that chose it: its popularity) and pricing (its price from the Recipe Pricer).',
  ...
  const planDescription =
    'Five dinners, Monday to Friday, from priced recipes, and the budget (the most a dinner may cost on average, US dollars per person). ' +
    'The coordinator looks up each dinner’s cost from the Recipe Pricer, adds up the week, and checks the budget and variety rules.';
  ...
  server.registerTool(
    'save_meal_plan',
    {
      ...
      description: `Save your group’s plan. ${planDescription} A plan that breaks a rule is still saved, with the rule marked as failed; check it first.`,
      inputSchema: planSchema,
    },
    async (input) => {
  ...
  server.registerTool(
    'list_meal_plans',
    {
      ...
      description: 'Every plan the Meal Planners saved, newest first, with its costs, the checks it passed and failed, and its dinners (with picked_by: the groups that chose each recipe).',
      inputSchema: z.strictObject({}),
      annotations: { readOnlyHint: true },
    },
    async () => {
  ...
  server.registerTool(
    'build_week_cart',
    {
      title: 'Build a plan’s Kroger cart',
      description:
        'The week’s Kroger cart for one saved plan, built from the Recipe Pricer’s carts: each product once, in whole packages, with what it is used for, and the total. ' +
        'Every product is checked with Kroger at the class store today (whether it still carries it, today’s price; not stock levels); "unavailable" lists what it no longer carries. ' +
        'To buy something else instead, search that store with search_kroger_products and pass replacements. Nothing is saved.',
      inputSchema: cartSchema,
      annotations: { readOnlyHint: true },
    },
    async (input) => {
      const res = await buildWeekCart(input);
      return res.ok ? reply(res.cart) : rejected(res.errors);
    },
  );
  ...
  server.registerTool(
    'save_choice',
    {
      ...
      description: 'Store the plan you choose for the class and why, with the same replacements you built its cart with. The coordinator builds and stores its Kroger cart; the newest choice is the class’s.',
    ...
    async (input) => {
      ...
      if (!res.ok) return rejected(res.errors);
      const c = res.choice;
      return reply({ saved: true, id: c.id, plan_id: c.plan_id, total_usd: c.total_usd, people: c.people });
    },
  );
