...
Start with get_expectations: how to work with the coordinator (formats, checks, limits, handoffs).
...
Limits per group apply to saves; get_expectations lists them.
...
const TOOLS = {
  scout: ['get_expectations', 'save_recipe'],
...
export function createMcpServer(group, { agent = null } = {}) {
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      return reply(await expectations(which));
    },
  );
