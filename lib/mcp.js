...
Shopper: compare the saved plans with list_meal_plans, read a plan's ingredients (the
Recipe Pricer's carts) with get_plan_ingredients, and store the class's choice and the
cart it built with save_choice; see /shopper.
...
export function createMcpServer(group, { agent = null } = {}) {
  ...
  const server = new SharedRulesMcpServer({ name: 'recipes', version: '2.1.0' }, { instructions: hello + INSTRUCTIONS });
  if (agent) {
  ...
  server.registerTool(
    'get_plan_ingredients',
    {
      ...
      description:
        'For each dinner in a saved plan, the Recipe Pricer’s cart: every ingredient it bought or estimated, as a need with a need_id, ' +
        'the product the Pricer chose and the share of one package the recipe uses for pricer_people. Nothing is saved.',
  ...
  server.registerTool(
    'save_choice',
    {
      ...
      description: 'Store the plan you choose for the class, why, and the week’s Kroger cart you built for it. The coordinator checks the cart covers every need of the plan exactly once and that its sums are right; the newest choice is the class’s.',
