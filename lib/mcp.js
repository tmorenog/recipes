...
const TOOLS = {
  ...
  shopper: ['get_contract', 'list_meal_plans', 'build_week_cart', 'save_choice'],
};
...
export function createMcpServer(group, { agent = null } = {}) {
  ...
  server.registerTool(
    'build_week_cart',
    {
      ...
      description: 'The week’s Kroger cart for one saved plan, built from the Recipe Pricer’s carts: each product once, in whole packages, with what it is used for, and the total. Nothing is saved.',
  ...
  server.registerTool(
    'save_choice',
    {
      ...
      description: 'Store the plan you choose for the class and why. The coordinator builds and stores its Kroger cart; the newest choice is the class’s.',
