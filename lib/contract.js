...
const CONTRACTS = {
  ...
  shopper: () => ({
    agent: 'Shopper',
    role: 'Chooses the class’s best meal plan from the plans the Meal Planners saved, and has the coordinator build its Kroger cart. The coordinator builds the cart from the Recipe Pricer’s carts and stores the choice.',
    tools: {
      get_contract: 'This description of how to work with the coordinator.',
      ...
      build_week_cart: 'The week’s Kroger cart for one plan and a number of people (default 50), without saving anything: each dinner scaled to that many people, every product to buy in whole packages, what it is used for, and the total. Every product is checked with Kroger at the class store today: whether the store still carries it and today’s price (not stock levels). Pass replacements to buy other products instead of ones it no longer carries.',
      search_kroger_products: 'Products matching a search at one Kroger store. Use the store_id in the cart’s "store" to find replacements.',
      save_choice: 'Stores the class’s choice: the plan, the number of people and why. The coordinator builds and stores its cart; the newest choice is the class’s active shopping plan.',
    },
    choice_format: fields(choiceSchema),
    data_sources: [
      'plan_id: the id of a saved plan from list_meal_plans.',
      'Costs and carts come from the Recipe Pricer and are added up by the coordinator. Never send costs or products, only replacements: a product_id in the cart and a replacement_product_id from search_kroger_products.',
      'Prices, and whether the store carries each product, are Kroger’s at the class store when the cart is built; the Pricer’s prices can be days old. Stock levels are not checked.',
    ],
    checks: {
      structure: ['Every field and type in choice_format; fields that are not listed are rejected.', 'The plan must exist, and every dinner in it must have a Kroger cart.', 'A replacement must be carried by the class store today, with a price; when the two package sizes don’t compare, give packages.'],
    },
    limits: ['None: build_week_cart saves nothing, and a later choice replaces an earlier one as the class’s.'],
    ...
    answers: {
      cart: '{ "plan_id", "group", "people", "lines": [{ "product_id", "description", "size", "price_usd", "packages", "cost_usd", "estimated", "used_for", "availability" (available, unavailable, estimated or unchecked), "priced_at_usd" (when today’s price differs), "substitutes" (the Pricer’s substitutes), "replaces" }], "unavailable": [what the store no longer carries], "store", "kroger_check", "total_usd", "per_person_usd" (the cart ÷ people, with whole packages), "plan_cost_per_person_usd" (the plan’s own cost, from shares of packages), "estimated_lines", "note" }',
      saved: '{ "saved": true, "id": …, "plan_id": …, "total_usd": … }',
      rejected: REJECTED,
    },
    example: { plan_id: '<id from list_meal_plans>', people: 50, reason: 'team-3’s plan passes every check at $2.40 a dinner, the cheapest of the three that do, and four of its five dinners were picked by more than one group.' },
  }),
};
