...
const CONTRACTS = {
  ...
  shopper: () => ({
    ...
    tools: {
      ...
      build_week_cart: 'The week’s Kroger cart for one plan, without saving anything: every product to buy, in whole packages, what it is used for, and the total.',
      save_choice: 'Stores the class’s choice: the plan and why. The coordinator builds and stores its cart; the newest choice is the class’s.',
    },
    ...
    answers: {
      cart: '{ "plan_id", "group", "people", "lines": [{ "description", "size", "price_usd", "packages", "cost_usd", "estimated", "used_for" }], "total_usd", "per_person_usd", "estimated_lines", "note" }',
      saved: '{ "saved": true, "id": …, "plan_id": …, "total_usd": … }',
      rejected: REJECTED,
    },
    example: { plan_id: '<id from list_meal_plans>', reason: 'team-3’s plan passes every check at $2.40 a dinner, the cheapest of the three that do, and four of its five dinners were picked by more than one group.' },
  }),
};
