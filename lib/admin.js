...
export async function updateRecipe(id, input) {
  ...
  const res = await getStore().updateRecipe(id, fields, { repriceStatus: auto_pricing ? 'pending' : 'unpriced' });
  if (res.duplicate) return fail(409, ['another recipe already has this meal_id: each TheMealDB recipe is stored once']);
  if (res.pickConflict) return fail(409, ['that group already picked this recipe for that theme: nothing was changed']);
  if (!res.row) return fail(404, [`no recipe has the id ${id}`]);
  await log('admin_edit_recipe', true, res.repriced ? `${id} (price dropped: queued again)` : id, input);
  // A changed meal_id, ingredients or servings needs a new price.
  if (res.repriced && auto_pricing) {
    const { kickPricer } = await import('./pricer.js');
    kickPricer();
  }
  return { ok: true, recipe: toRecipe(res.row), repriced: Boolean(res.repriced) };
}
...
export async function clearChoices() {
  const n = await getStore().clearChoices();
  await log('admin_clear_choices', true, `class plan cleared (${n} choice${n === 1 ? '' : 's'})`);
...
export async function restore(input) {
  if (input?.format !== BACKUP_FORMAT) return fail(400, ['this isn’t a Meal Squad backup file (download one with “Download backup”)']);
...
export async function reset(input) {
  ...
  await log('admin_reset', true, 'deleted all recipes, meal plans, the class’s plan and activity');
  return { ok: true };
}
...
// For admin actions logged from outside this file.
export const logAdmin = (action, detail) => log(action, true, detail);
