...
export function supabaseDb(
  ...
  return {
    ...
    // For /api/health: can we reach each table? A missing table means the
    // migration hasn't been run.
    async tableStatus(tables) {
      const out = {};
      for (const t of tables) {
        const { count, error } = await sb.from(t).select('*', { count: 'exact', head: true });
        out[t] = error ? { ok: false, error: error.message || error.code || 'unreachable' } : { ok: true, rows: count };
      }
      return out;
    },
