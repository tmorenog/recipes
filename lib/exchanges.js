...
const TOOL_AGENT = {
  ...
  list_meal_plans: 'shopper',
  get_plan_ingredients: 'shopper',
  save_choice: 'shopper',
};
...
function requestSummary(tool, args = {}) {
  if (tool === 'get_plan_ingredients') return `plan ${String(args.plan_id ?? '').slice(0, 8)}`;
  if (tool === 'save_choice') {
    const lines = Array.isArray(args.cart?.lines) ? args.cart.lines.length : 0;
    return `plan ${String(args.plan_id ?? '').slice(0, 8)}, a cart of ${lines} product${lines === 1 ? '' : 's'}${args.cart?.people ? ` for ${args.cart.people} people` : ''}, ${money(args.cart?.total_usd)}`;
  }
...
function answerSummary(tool, json, text) {
  ...
  switch (tool) {
    case 'get_contract':
      return `the ${json.agent || 'agent'}’s contract: role, ${Object.keys(json.tools || {}).length} tools, formats, checks and ${(json.limits || []).length} limits`;
    case 'save_recipe':
    ...
    case 'list_meal_plans':
      return `${json.count} plan${json.count === 1 ? '' : 's'}`;
    case 'get_plan_ingredients': {
      const needs = (json.dinners || []).reduce((n, d) => n + (d.needs?.length ?? 0), 0);
      return `${json.group}’s plan: ${json.dinners?.length ?? '?'} dinners, ${needs} ingredients to buy, from the Recipe Pricer’s carts`;
    }
    case 'save_choice':
      return `cart checked and stored: the class’s plan, ${money(json.total_usd)}${json.people ? ` for ${json.people} people` : ''}`;
    case 'mark_processed':
...
export function describe(message, response, { agent = null } = {}) {
  ...
  if (method === 'tools/list') {
    ...
    return {
      ...
      summary: error ? cut(redact(error.message || 'error')) : `${names.length} tools: ${names.join(', ')}`,
    };
  }
  ...
  return { agent, method, tool: null, ok: !error, request: small(message.params ?? null), response: small(error ?? response?.result ?? null), summary: error ? cut(redact(error.message || 'error')) : 'done' };
}

// Logging never breaks a request: a failure here is only reported.
export async function logExchange(entry) {
  try {
    await getStore().logExchange({
      ...entry,
      summary: entry.summary == null ? entry.summary : redact(entry.summary),
      request_summary: entry.request_summary == null ? entry.request_summary : redact(entry.request_summary),
    });
  } catch (e) {
