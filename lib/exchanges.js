...
function requestSummary(tool, args = {}) {
  if (tool === 'build_week_cart' || tool === 'save_choice') return `plan ${String(args.plan_id ?? '').slice(0, 8)}`;
...
function answerSummary(tool, json, text) {
  ...
  switch (tool) {
    ...
    case 'build_week_cart':
      return `cart for ${json.group}’s plan: ${json.lines?.length ?? '?'} products, ${money(json.total_usd)}${json.people ? ` for ${json.people} people` : ''}`;
    case 'save_choice':
      return `chose a plan: its cart costs ${money(json.total_usd)}`;
    case 'mark_processed':
