...
export const startSchema = z.discriminatedUnion('agent', [
  ...
  z.strictObject({ agent: z.literal('shopper'), group: group.default('shopper') }),
]);
...
function brief_(agent, input) {
  if (agent === 'scout') {
    return {
      system: `You are the Scout Agent in Meal Squad, a class system of agents that plans five dinners, Monday to Friday. You find real recipes on TheMealDB that fit a theme and save them to the coordinator, so the Pricer Agent can price them and the Planner Agents can use them.
...
- Choose five varied recipes that fit the theme, and say briefly why each fits.
...
- When five recipes have been accepted, or if you can't get five accepted, call finish with a short summary.`,
      first: `The theme: ${input.theme}`,
    };
  }
  if (agent === 'shopper') {
    return {
      system: `You are the Shopper Agent in Meal Squad, a class system of agents. The Meal Planners have saved meal plans of five dinners. You choose the one the class will cook, and the coordinator builds its Kroger cart.
...
- Build the Kroger cart of your top choice, and of a close second if it helps you decide, before choosing.
- Save your choice once, with a reason that compares it with the others, then call finish with a short summary.
- If there are no plans, or none has a cart yet, save nothing and explain why.
- Never invent a plan, a price or a number.`,
      first: 'Choose the class’s meal plan.',
    };
  }
  return {
    system: `You are the Planner Agent in Meal Squad, a class system of agents. You plan five dinners, Monday to Friday, from the recipes the Scout Agents found and the Pricer Agent priced, and save the plan to the coordinator.
...
async function runAgent(id, agent, groupName, input) {
  ...
  try {
    ...
    const { system, first } = brief_(agent, input);
    ...
    while (true) {
      ...
      for (const use of uses) {
        ...
        try {
          if (use.name === 'finish') {
            ...
            ({ text, isError } = await coord.call(use.name, use.input));
      ...
      if (agent === 'scout' && accepted >= 5) return end('done', 'Five recipes saved', finished || 'The coordinator accepted five recipes.');
      if (finished != null) {
