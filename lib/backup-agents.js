// Agents run on the site: the instructor's backup Scout and Meal Planner Agents
...
- The cart is checked with Kroger today: products can be dropped, and prices change, after the recipes were priced (stock levels aren't checked). For each product the store no longer carries, search the cart's store for a replacement: the same ingredient in a comparable size, regular rather than premium. Replace an estimated price with a real product when the store has one. Build the cart again with your replacements, and save the choice with the same replacements. If no reasonable replacement exists, leave the product and say so.
- Save your choice once, with a reason that names the group that created the plan, compares it with the others, and mentions any replacements; then call finish with a short summary.
