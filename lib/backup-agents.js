// Backup agents: the instructor's own Scout Agent and Planner Agent, run from
// the site when a group's Lovable app isn't working. They work the way the
...
export const LIMITS = { scout: 35, planner: 30, runMs: 240_000 }; // actions per run; a function may run 5 minutes
...
export async function startBackupRun(raw) {
  ...
  if (created.busy) return { ok: false, status: 409, errors: [`A backup ${input.agent === 'scout' ? 'Scout' : 'Planner'} Agent is already running. Wait for it to finish.`] };
