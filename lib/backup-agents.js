...
- Then shop like a person would. The Pricer priced each recipe on its own, so the same ingredient can be bought as different products for different dinners (yellow onions in a 3 lb and a 1 lb bag, two brands of olive oil). Buy one product for all of them when they're really the same ingredient, usually the better value per unit; keep them apart when they differ (red and yellow onions, fresh and ground ginger). For each product the store no longer carries, search for a replacement: the same ingredient in a comparable size, regular rather than premium. Replace an estimated price with a real product when the store has one. Stock levels aren't checked.
- Review the cart, then save your choice once, with a reason that names the group that created the plan, compares it with the others, and mentions what you combined or replaced. Then call finish with a short summary.
