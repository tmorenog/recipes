// How to work with the coordinator, served by the MCP tool get_expectations.
...
const BRIEFS = {
  scout: (s) => ({
    ...
    tools: {
      get_expectations: 'This description of how to work with the coordinator.',
  ...
  planner: (s) => ({
    ...
    data_sources: [
      ...
      'nutrition_per_serving: your estimate for one serving; the coordinator stores it as the Meal Planner’s estimate.',
    ...
    checks: {
      ...
      reported_for_every_plan: {
        note: 'The class’s rules for a plan (classroom rules, not dietary advice). check_meal_plan and save_meal_plan report each one as passed or not.',
    ...
    answers: {
      checked: '{ "week_cost_per_person_usd": …, "budget_usd": …, "all_rules_passed": true|false, "checks": [{ "rule", "passed", "detail" }], "meals": [each dinner with its cost] }',
    ...
    example: {
      budget_usd: 15,
      summary: 'Five dinners from four cuisines for $11.40 per person. Tuesday is the vegetarian night.',
      meals: [
        {
          day: 'Monday',
...
export const expectations = async (agent) => BRIEFS[agent](await getSettings());
