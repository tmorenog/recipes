...
function product(p) {
  ...
  return {
    ...
    // Kroger's rough stock level at the store, when it says: HIGH, LOW or
    // TEMPORARILY_OUT_OF_STOCK. Not a count, and not always reliable.
    stock: item.inventory?.stockLevel || null,
    image_url: imageUrl(p),
  };
}

// A product at a store today: whether the store carries it, today's price and
// Kroger's rough stock level (cached for a few minutes only). Returns
// { ok, product } with product null when the store doesn't have it, or
