...
export async function lenientPlan(input) {
  ...
  value.meals = value.meals.map((m, i) => {
    ...
    // Nutrition isn't part of the exercise: an estimate sent anyway is dropped, not rejected.
    if ('nutrition_per_serving' in out) {
      delete out.nutrition_per_serving;
      notes.push(`meals.${i}.nutrition_per_serving isn’t used and was left out`);
    }
    return out;
  });
  return { value, notes };
