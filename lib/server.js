...
Recipe Scout: find recipes and store each one with add_recipe.
...
export function createMcpServer({ db, caller = 'unknown' }) {
  ...
  server.registerTool(
    ...
    async (recipe) => {
      const res = await db.addRecipe(recipe, caller === 'unknown' ? 'recipe-scout' : caller);
      if (res.duplicate) {
        const error = `a recipe with meal_id "${recipe.meal_id}" is already saved for theme "${recipe.theme}"`;
        await log('add_recipe', false, recipe, error);
        return rejected([error]);
      }
      await log('add_recipe', true, recipe);
