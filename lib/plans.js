...
export function ruleText(c) {
  return {
    ...
    calories: `Every dinner has ${c.calories_min} to ${c.calories_max} calories per serving`,
    protein: `Every dinner has at least ${c.protein_min_g} g protein per serving`,
    fiber: `Every dinner has at least ${c.fiber_min_g} g fibre per serving`,
    sodium: `Every dinner has less than ${c.sodium_max_mg.toLocaleString('en-US')} mg sodium per serving`,
...
export async function evaluatePlan(input) {
  ...
  const [lo, hi, protein_g, fiber_g, sodium_mg] = [c.calories_min, c.calories_max, c.protein_min_g, c.fiber_min_g, c.sodium_max_mg];
  ...
  return { ok: true, plan, meals, week, checks, passed: checks.every((c) => c.passed), notes };
}
