...
export const mealSchema = z.strictObject({
  ...
  nutrition_per_serving: nutritionSchema.describe('Your estimate for one serving, worked out from the recipe’s ingredients (search_foods gives USDA values per 100 g)'),
});
...
export async function evaluatePlan(input) {
  ...
  const prices = await store.pricingsFor([...new Set([...recipes.values()].map((r) => r.meal_id))]);
  for (const id of new Set(ids)) {
    const r = recipes.get(id);
    if (!r) errors.push(`meals: no recipe has the id ${id} (use ids from list_recipes)`);
    else if (pricingSummary(prices.get(r.meal_id)).status !== 'priced') errors.push(`meals: “${r.name}” isn’t priced yet; choose recipes whose pricing.status is "priced"`);
  }
  ...
  const meals = [...plan.meals]
    ...
    .map((m) => {
      ...
      const p = pricingSummary(prices.get(r.meal_id));
      return {
...
const answer = (e) => ({
  ...
  checks: e.checks,
  meals: e.meals.map(({ day, recipe_id, meal_id, name, cuisine, category, image_url, why, cost_per_serving_usd, nutrition_per_serving }) =>
    ({ day, recipe_id, meal_id, name, cuisine, category, image_url, why, cost_per_serving_usd, nutrition_per_serving })),
});
