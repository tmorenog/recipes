...
const nutritionSchema = z.strictObject({
  calories: z.number().finite().min(0).max(5000).describe('Estimated calories per serving'),
  protein_g: z.number().finite().min(0).max(500).describe('Estimated grams of protein per serving'),
  fiber_g: z.number().finite().min(0).max(200).describe('Estimated grams of fibre per serving'),
  sodium_mg: z.number().finite().min(0).max(20000).describe('Estimated milligrams of sodium per serving'),
});
...
export const mealSchema = z.strictObject({
  ...
  servings: z.number().int().min(1).max(100).describe('How many people this meal serves: the number you were given'),
  cost_used_usd: money.describe('What the ingredients this meal actually uses cost'),
  cost_per_serving_usd: money.describe('cost_used_usd ÷ servings, rounded to cents'),
  nutrition_per_serving: nutritionSchema.describe('Your estimates: { calories, protein_g, fiber_g, sodium_mg }'),
});

export const shoppingItemSchema = z.strictObject({
  item: text(200).describe('What to buy, e.g. "yellow onions"'),
  kroger_product_id: optionalText(50).describe('product_id from search_kroger_products'),
  ...
  unit_price_usd: money.describe('Price of one package: promo_price_usd when there is one, else price_usd'),
...
export const planSchema = z.strictObject({
  ...
  budget_usd: money.nullable().optional().describe('The budget you were given, in US dollars'),
  ...
  meals: z.array(mealSchema).length(5).describe('Exactly 5 meals, one per day: see meal_format'),
  shopping_list: z.array(shoppingItemSchema).min(1).max(200).describe('One line per Kroger product: see shopping_item_format'),
  rule_checks: z
    .array(z.strictObject({ rule: text(200), passed: z.boolean(), detail: z.string().max(1000).optional() }))
    .max(50)
    .optional()
    .describe('One entry per balance rule: { rule, passed (true/false), detail }'),
});
