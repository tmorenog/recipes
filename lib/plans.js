...
// rules, all from the stored data (cost, cuisine, category).
...
export async function evaluatePlan(input) {
  ...
  const meals = [...plan.meals]
    ...
    .map((m) => {
      ...
      const pickedBy = [...new Set((r.picks || []).map((x) => x.group))];
      return {
        day: m.day,
        ...
        pick_count: pickedBy.length,
        picked_by: pickedBy, // the groups whose Scout Agents chose this recipe, first first
      };
    });
...
const answer = (e) => ({
  ...
  checks: e.checks,
  meals: e.meals.map(({ day, recipe_id, meal_id, name, cuisine, category, image_url, why, cost_per_serving_usd, price_estimated, pick_count, picked_by }) =>
    ({ day, recipe_id, meal_id, name, cuisine, category, image_url, why, cost_per_serving_usd, price_estimated, pick_count, picked_by })),
...
export async function savePlan({ group, channel, input }) {
  ...
  }, { maxPlans: (await getSettings()).max_plans_per_group });
  if (row.limit != null) {
    const limited = totalLimit('plan', row.limit, (await getSettings()).max_plans_per_group);
    await log(false, limited.errors[0]);
    return { ok: false, ...limited };
  }
