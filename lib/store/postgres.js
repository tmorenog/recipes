  ...
  return {
    ...
    async saveRecipePick(r) {
      ...
      try {
        ...
        const row = (await run(pool, `select ${COLUMNS} from recipes where id = $1`, [id])).rows[0];
        return { row, created };
      } catch (e) {
    ...
    async updateRecipe(id, fields) {
      ...
      try {
        const { rows } = await run(pool, `update recipes set ${sets} where id = $1 returning id`, [id, ...values]);
        if (!rows[0]) return { row: null };
        ...
        return { row: (await run(pool, `select ${COLUMNS} from recipes where id = $1`, [id])).rows[0] };
      } catch (e) {
    ...
    async replaceAll({ recipes, plans, activity }) {
      ...
      try {
        await client.query('begin');
        await client.query('delete from activity; delete from plans; delete from recipe_picks; delete from recipes; delete from pricer_steps; delete from pricings;');
    ...
    async enqueuePricing(mealId) {
      // A no-op update on conflict copies an existing price onto a re-saved recipe.
      await run(pool, 'insert into pricings (meal_id) values ($1) on conflict (meal_id) do update set meal_id = excluded.meal_id', [mealId]);
    },
    ...
    // ------------------------------------------------------------ Pricer test runs
    async createPricerTest({ ingredients, serves, prompt, draft }) {
      ...
      const { rows } = await run(
        pool,
        'insert into pricer_tests (ingredients, serves, prompt, draft) values ($1, $2, $3, $4) returning *',
        [JSON.stringify(ingredients), serves, prompt, draft],
      );
      return rows[0];
    },
