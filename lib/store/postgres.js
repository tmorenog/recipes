...
export function postgresStore(urlOrPool) {
  ...
  return {
    ...
    async pricingsFor(mealIds) {
      ...
      const { rows } = await run(
        pool,
        `select p.meal_id, p.status, p.total_cost_usd, p.to_buy_usd, p.people, p.error, (select count(*) from jsonb_array_elements(p.basket) e where e->>'status' = 'estimated')::int as estimated_lines from pricings p where p.meal_id = any($1)`,
        [mealIds],
      );
