...
export function postgresStore(urlOrPool) {
  ...
  return {
    ...
    // The class's plan, its earlier choices and the Shopper Agent's finished runs.
    async clearChoices() {
      const { rowCount } = await run(pool, 'delete from class_choices');
      await run(pool, "delete from backup_runs where agent = 'shopper' and status <> 'running'");
      return rowCount;
    },
    ...
    async replaceAll({ recipes, plans, activity }) {
      ...
      try {
        await client.query('begin');
        await client.query("delete from activity; delete from exchanges; delete from class_choices; delete from backup_runs where status <> 'running'; delete from plans; delete from recipe_picks; delete from recipes; delete from pricer_steps; delete from pricings;");
