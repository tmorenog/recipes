...
export const pricerSettings = (env = process.env) => ({
  aiKey: setting(['ANTHROPIC_API_KEY'], env)?.trim().replace(/^(["'])(.*)\1$/, '$2').trim() || null,
...
export const AI_KEY_WRONG =
  'ANTHROPIC_API_KEY doesn’t look like an API key: in Vercel it must be only the key, one line starting “sk-ant-”, with no command, quotes or spaces. Fix it and redeploy.';
...
export function pricerProblem() {
  if (!overrides.model) {
    ...
    if (!aiKeyLooksRight(key)) return AI_KEY_WRONG;
  }
...
export const DEFAULT_PROMPT = `You are the Pricer, an agent that runs on the class's recipe coordinator. For one recipe, you build the Kroger shopping cart needed to cook it and find what it costs, so the Meal Planner agents can compare recipes by price.
...
2. Decide how much of that product the scaled recipe uses, as amount_used and unit_used (g, kg, oz, lb, ml, l, tsp, tbsp, cup, fl oz or each). When the recipe measures by volume and the product is sold by weight (or the other way round), also estimate the weight in grams.
3. Call record_ingredient with the product_id and the amounts. Code works out how many packages to buy and the cost: never calculate them yourself.

Skip a line only when nothing is bought: water, or an ingredient with no amount such as "salt, to taste".
If a tool returns an error, fix what it says and try again. Never invent a product or a price: use only what the searches return.
...
async function agentLoop({ recipe, shop, prompt, saved = null, toolCallsSoFar = 0, deadline, log: rawLog, save = async () => {} }) {
  ...
  while (true) {
    ...
    if (finished) {
      ...
      await log({ kind: 'final', text: finished.summary, output: { people: finished.people, cart_usd: t.toBuy, cost_used_usd: t.total } });
...
async function runTest(id, { ingredients, serves, prompt }) {
  ...
  if (result.outcome === 'priced') {
    return store.updatePricerTest(id, {
      ...
      totals: { cart_usd: result.totals.toBuy, cost_used_usd: result.totals.total },
    });
  }
