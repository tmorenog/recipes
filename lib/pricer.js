...
// save, it finds every ingredient at the class's Kroger store and builds the
// shopping cart and its cost. (Nutrition is the Meal Planner's job.) The AI
...
export function pricerProblem() {
  if (!overrides.model) {
    ...
    if (!key) return 'The Pricer needs an AI key: the instructor should add ANTHROPIC_API_KEY in Vercel.';
...
export const DEFAULT_PROMPT = `You are the Pricer, an agent that runs on the class's recipe coordinator. For one recipe at a time, you build the Kroger shopping cart needed to cook it and find out what it costs, so the Meal Planners can compare recipes by price. You only price: nutrition and taste are other agents' work.

We are buying for 50 people. Scale the recipe's quantities from the number of people it serves to 50.

Your tools are described in their own definitions: read them and decide which to call, and in what order. They search one Kroger store through Kroger's Products API (documentation: https://developer.kroger.com/documentation/public/product/overview), and they do the arithmetic, turning the product and amount you choose into packages to buy and a cost. For example (only an example), you might search for "canned chickpeas", pick a 15 oz can, and record that the recipe uses 5 kg of it.

What a good cart looks like:
- Every ingredient line is accounted for: priced from Kroger where possible; estimated, with the reason, only when Kroger has no suitable product or no price; or skipped when nothing is bought (water, "salt, to taste").
- Products match what the recipe means (fresh garlic, not garlic powder), in package sizes that suit the amount. Regular products rather than premium ones, unless nothing else fits.
- The tools do the maths; never calculate costs yourself, and never present an invented product or price as Kroger's.
...
async function runTool(name, input, ctx) {
  ...
  switch (name) {
    ...
    case 'record_ingredient': {
      ...
      return { result: { recorded: d.line, cost_used_usd: e.cost_used_usd, packages_to_buy: e.packages, cost_to_buy_usd: e.cost_to_buy_usd, note: e.note, remaining_lines: remaining() } };
    }
...
async function priceOne(claimed, deadline) {
  ...
  try {
    ...
    return store.finishPricing(claimed.meal_id, { status: 'failed', error: redact(e.message) });
  }
  ...
  const result = await agentLoop({
    ...
    deadline,
    log,
    save: (st) => store.savePricingProgress(claimed.meal_id, {
  ...
  if (result.outcome === 'paused') return store.releasePricing(claimed.meal_id); // the next run carries on
  if (result.outcome === 'failed') return store.finishPricing(claimed.meal_id, { status: 'failed', error: result.error, basket: result.basket });
  const t = result.totals;
  return store.finishPricing(claimed.meal_id, {
    status: 'priced', summary: result.summary, people: result.people, prompt, basket: result.basket, total: t.total, toBuy: t.toBuy,
  });
}
...
export async function startTest(input) {
  ...
  const { ingredients, serves } = parsed.data;
  const prompt = parsed.data.prompt || (await currentPrompt());
  const store = getStore();
  const test = await store.createPricerTest({ ingredients, serves, prompt, draft: Boolean(parsed.data.prompt) && parsed.data.prompt !== (await currentPrompt()) });
  const run = runTest(test.id, { ingredients, serves, prompt }).catch(async (e) => {
...
async function runTest(id, { ingredients, serves, prompt }) {
  const store = getStore();
  ...
  const recipe = { name: 'Sample shopping list', est_servings: serves, ingredients: ingredients.map((line) => ({ name: line, raw: null, listed: true })) };
  await log({ kind: 'thought', text: `Test run: pricing ${ingredients.length} ingredients at ${shop.name} (store ${shop.store_id}) with ${pricerSettings().model}.` });
  const result = await agentLoop({
    recipe,
    shop,
    prompt,
