...
async function agentLoop({ recipe, shop, prompt, saved = null, toolCallsSoFar = 0, deadline, log, save = async () => {} }) {
...
async function priceOne(claimed, deadline) {
  ...
  try {
    ...
    await log({ kind: 'error', text: e.message });
...
export async function startTest(input) {
  ...
  const run = runTest(test.id, { ingredients, serves, prompt }).catch(async (e) => {
    ...
    await store.updatePricerTest(test.id, { status: 'failed', error: e.message });
  });
  try {
    waitUntil(run);
  } catch {
...
async function runTest(id, { ingredients, serves, prompt }) {
  ...
  try {
    ...
    await log({ kind: 'error', text: e.message });
    return store.updatePricerTest(id, { status: 'failed', error: e.message });
  }
