...
export function createShopper({ people, coordCall, fetchImpl = () => pricer.krogerFetch() }) {
  ...
  function put(product, shares, { estimated = false, note, extra, pricedAt } = {}) {
    const line = lines.get(product.product_id) ?? {
      ...
      price_usd: product.price_usd, image_url: product.image_url ?? null, estimated, shares: new Map(), notes: [],
    };
  ...
  const tools = {
    ...
    async buy_as_priced({ needs: which } = {}) {
      ...
      for (const [productId, group] of groups) {
        ...
        put({ ...p, product_id: productId, price_usd: now, image_url: t.product.image_url ?? p.image_url }, shares, { pricedAt: p.price_usd });
      }
      return {
    ...
    async buy({ items }) {
      ...
      for (const item of items) {
        ...
        results.push({ product_id: p.product_id, description: p.description, size: p.size, price_usd: p.price_usd, packages: line.packages, cost_usd: line.cost_usd, needs: [...line.shares.keys()] });
      }
    ...
    async review_cart() {
      if (!plan) return { error: 'start_cart first' };
      return { plan_id: plan.plan_id, people, lines: [...lines.values()].map(lineView), still_needed: uncovered(), total_usd: total() };
    },
  };
  ...
  return {
    ...
    cart() {
      ...
      return {
        ...
        kroger_check: s
          ? `The Shopper Agent checked every product with Kroger at ${s.name ?? 'the class store'} today${changed.size ? `; ${changed.size} price${changed.size === 1 ? '' : 's'} changed since pricing` : ''}. ${count((l) => l.shares.size > 1)} product${count((l) => l.shares.size > 1) === 1 ? ' is' : 's are'} shared by several dinners and bought once. Double-check stock quantities before shopping: buying for many people can empty a shelf.`
