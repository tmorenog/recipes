  ...
  describe(`with the ${backend.name} store`, { skip: backend.skip }, () => {
    ...
    test('MCP publishes the tools, with their input formats', async () => {
      ...
      assert.deepEqual(tools.map((t) => t.name).sort(), [
        'build_week_cart', 'check_meal_plan', 'find_kroger_stores', 'get_contract', 'list_meal_plans', 'list_recipes', 'mark_processed', 'save_choice', 'save_meal_plan', 'save_recipe', 'search_kroger_products',
      ]);
    ...
    test('meal plans use priced recipes; the coordinator adds up the week and checks the rules', async () => {
      ...
      assert.deepEqual(failed.map((c) => c.rule), ['The average dinner costs no more than the budget, per person']);
      assert.match(failed[0].detail, /\$2\.55 a dinner on average \(\$12\.75 for the week\), budget \$2\.50 a dinner/);
      assert.equal(checked.average_dinner_cost_per_person_usd, 2.55);
